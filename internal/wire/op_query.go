...
func (query *OpQuery) MarshalBinary() ([]byte, error) {
	...
	if err := bson.NewDocument(query.Query).WriteTo(bufw); err != nil {
		return nil, err
	}

	if query.ReturnFieldsSelector != nil {
		if err := bson.NewDocument(query.ReturnFieldsSelector).WriteTo(bufw); err != nil {
			return nil, err
		}
	}
...
func (query *OpQuery) MarshalJSON() ([]byte, error) {
	m := map[string]interface{}{
		...
		"Query":              bson.NewDocument(query.Query),
	}
	if query.ReturnFieldsSelector != nil {
		m["ReturnFieldsSelector"] = bson.NewDocument(query.ReturnFieldsSelector)
	}
